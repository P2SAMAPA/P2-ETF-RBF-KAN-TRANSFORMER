...
st.set_page_config(page_title="RBF-KAN-Transformer Engine", layout="wide")
...
.badge-strongsell{background:#7a1a1a;border-radius:6px;padding:2px 12px;font-size:0.75rem;
                  font-weight:700;color:white}
.badge-rbf{background:#8e44ad;border-radius:6px;padding:2px 8px;font-size:0.65rem;
           font-weight:700;color:white}
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="main-header">🌀 RBF-KAN-Transformer Engine</div>',
            unsafe_allow_html=True)
st.markdown(
    '<div class="sub-header">RBF-KAN-Transformer · Hybrid Neural Architecture · '
    'Kolmogorov-Arnold Networks · Financial Forecasting</div>',
    unsafe_allow_html=True)
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
st.sidebar.markdown("## 🌀 RBF-KAN-Transformer")
st.sidebar.markdown(f"**Next Trading Day**")
st.sidebar.markdown(f"`{next_trading_day()}`")
st.sidebar.markdown(f"**RBF Centers:** {config.MODEL['rbf_centers']}")
st.sidebar.markdown(f"**KAN Layers:** {config.MODEL['kan_layers']}")
st.sidebar.markdown(f"**Transformer Heads:** {config.MODEL['transformer_heads']}")
st.sidebar.markdown(f"**Hidden Dim:** {config.MODEL['transformer_dim']}")
st.sidebar.markdown("---")
st.sidebar.markdown("**Architecture:**")
st.sidebar.markdown("  • RBF (Localized Patterns)")
st.sidebar.markdown("  • KAN (Non-linear Features)")
st.sidebar.markdown("  • Transformer (Global Context)")
st.sidebar.markdown("---")
...
tab1_path = find_latest(files, "rbf_kan_transformer_")
tab2_path = find_latest(files, "rbf_kan_transformer_breakdown_")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.header("🏆 Best Window per ETF — RBF-KAN-Transformer Signal")

    with st.expander("📖 How RBF-KAN-Transformer Works", expanded=True):
        st.markdown("""
**RBF-KAN-Transformer** is a hybrid neural architecture:

| Component | What it does |
|-----------|--------------|
| **RBF Layer** | Localized pattern recognition with Gaussian kernels |
| **KAN Layer** | Interpretable non-linear feature learning (Kolmogorov-Arnold) |
| **Transformer** | Self-attention for global context |
| **Forecast** | Future return prediction |

**Interpretation:**
- **Higher z-score** → Stronger forecast signal
    ...
    for universe_name in UNIVERSE_ORDER:
        ...
        if buy_etfs:
            ...
            for idx, (ticker, z_score, data) in enumerate(buy_etfs[:3]):
                ...
                forecast = safe_float(data.get("forecast", 0))
...
<div class="hero-card">
  ...
  <div class="score">Forecast = {forecast:.4f}</div>
...
""", unsafe_allow_html=True)
            ...
            if full_scores:
                ...
                for t, info in full_scores.items():
                    ...
                    rows.append({
                        "ETF": t,
                        "z-score": round(z, 4),
                        "Forecast": round(safe_float(info.get("forecast", 0)), 4),
                        "Loss": round(safe_float(info.get("loss", 0)), 4),
                        "N Epochs": int(safe_float(info.get("n_epochs", 0))),
                ...
                styled_df = df_rank.style.map(
                    lambda x: 'background-color: #27ae60; color: white;' if isinstance(x, (int, float)) and x > 0.05 else '',
                    subset=['z-score']
                ).map(
                    lambda x: 'background-color: #f1c40f; color: black;' if isinstance(x, (int, float)) and -0.05 < x <= 0.05 else '',
                    subset=['z-score']
                ).map(
                    lambda x: 'background-color: #e74c3c; color: white;' if isinstance(x, (int, float)) and x <= -0.05 else '',
                    subset=['z-score']
                )
    ...
    st.caption(f"Run date: {data1.get('run_date','?')} · Higher z-score = stronger forecast")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.header("🔍 Explore RBF-KAN-Transformer by Window")
    ...
    with st.expander("ℹ️ Window guidance", expanded=False):
        st.markdown("""
- **63d** — Short-term RBF-KAN-Transformer forecasting
- **126d** — Medium-term forecasting
- **252d** — Annual forecasting: recommended primary
- **504d** — Long-term forecasting
        """)

    st.markdown(f"### RBF-KAN-Transformer Rankings at **{win_labels.get(selected_win, f'{selected_win}d')}** window")
