"""
trainer.py  —  Orchestrator for RBF-KAN-Transformer Engine
===========================================================

Loads data → trains RBF-KAN-Transformer → forecasts → ranks ETFs → builds JSON.
"""
...
from rbf_kan_transformer import compute_universe_rbf_kan
...
def run_trainer(hf_token: Optional[str] = None) -> Dict:
        ...
        for ticker in available:
            ...
            if best_win is not None:
                best_window_per_etf[ticker] = {
                    "z_score": best_z,
                    "window": int(best_win),
                    "forecast": safe_float(best_data.get("forecast", 0)),
                    "signal": safe_float(best_data.get("signal", 0)),
                    "loss": safe_float(best_data.get("loss", 0)),
                    "n_epochs": int(safe_float(best_data.get("n_epochs", 0))),
    ...
    logger.info("\n💾 Saving JSON results...")
    tab1_path = f"rbf_kan_transformer_{run_date}.json"
    tab2_path = f"rbf_kan_transformer_breakdown_{run_date}.json"
